...
public:
  ...
  //   raw_status(): raw integer status from pclose()/wait (platform-specific)
  ...
  //   last_error(): a short string describing a start() failure (e.g., popen)
  ...
  //   - Builds the exact shell command string and launches it via popen().
  ...
  // Returns:
  //   true on success (pipe established), false on failure (with last_error()).
  ...
  // stop
  //   Closes the pipe to Spike if open, recording the process wait status
  //   (available via status helpers), and closes the raw log file if open.
  void stop();
...
private:
  ...
  // The pipe to Spike's merged stdout/stderr. Non-null while running.
  FILE* pipe_ = nullptr;
  ...
  // Last observed wait status from pclose(), and flags for status validity.
  int last_status_ = 0;
